...
class BoTimKiemGiaoDich:
    ...
    def Cat_chuoi(self, chuoi, vi_tri_dau, vi_tri_cuoi):
    ...
    def Kiem_tra_chuoi(self, chuoi_lon, chuoi_con):
        # Kiểm tra xem chuỗi con có xuất hiện trong chuỗi lớn hay không
        len_lon = self.bo_gd.Dem_phan_tu(chuoi_lon)
        len_con = self.bo_gd.Dem_phan_tu(chuoi_con)
        ...
        for i in range(len_lon - len_con + 1):
            doan_cat = self.Cat_chuoi(chuoi_lon, i, i + len_con)
    ...
    def Tim_kiem_giao_dich(self, so_dien_thoai, tu_khoa_nd=None, ma_gd_can_tim=None, ngay_bat_dau=None, ngay_ket_thuc=None):
        ...
        lich_su_goc = self.bo_gd.Lay_lich_su(so_dien_thoai)
        mang_ket_qua = []
        
        so_luong_gd = self.bo_gd.Dem_phan_tu(lich_su_goc)
        ...
        for i in range(so_luong_gd):
            ...
            if ma_gd_can_tim is not None:
                ma_tim_str = self.bo_gd.Bien_doi_thanh_chuoi(ma_gd_can_tim)
                if gd.ma_gd != ma_tim_str:
                    hop_le = False
            
            if hop_le and (ngay_bat_dau is not None or ngay_ket_thuc is not None):
                ngay_cua_gd = self.Cat_chuoi(gd.ma_gd, 2, 10)
                
                if ngay_bat_dau is not None:
                    ngay_bd_str = self.bo_gd.Bien_doi_thanh_chuoi(ngay_bat_dau)
                    if ngay_cua_gd < ngay_bd_str:
                        hop_le = False
                        
                if hop_le and ngay_ket_thuc is not None:
                    ngay_kt_str = self.bo_gd.Bien_doi_thanh_chuoi(ngay_ket_thuc)
                    if ngay_cua_gd > ngay_kt_str:
                        hop_le = False
                    
            if hop_le and tu_khoa_nd is not None:
                tk_str = self.bo_gd.Bien_doi_thanh_chuoi(tu_khoa_nd)
                if not self.Kiem_tra_chuoi(gd.noi_dung, tk_str):
                    hop_le = False
    ...
    def Hien_thi_giao_dich(self, danh_sach_gd, tieu_de=""):
        ...
        for idx, gd in enumerate(danh_sach_gd, 1):
            ngay = self.Cat_chuoi(gd.ma_gd, 2, 10)
            gio = self.Cat_chuoi(gd.ma_gd, 11, 17)
    ...
    while True:
        ...
        try:
            ...
            if lua_chon == "0":
                ...
                lich_su = bo_tim_kiem.bo_gd.Lay_lich_su(so_dien_thoai)
                ...
                if len(ket_qua) == 1:
                    ...
                    if xem_ct == 'y':
                        bo_tim_kiem.Hien_thi_chi_tiet(ket_qua[0])
